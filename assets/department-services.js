(function(){
  ...
  var SERVICES = {
    ...
    "recreation":[
      ["Support youth development","Offers youth-focused leagues and activities that build healthy habits and skills."],
  ...
  var SNAPSHOT_CHART_CATEGORIES=[
    ...
    // Grants and Aid, Other Uses, and Debt Service are real top-level
    // Object_Types too (Statutory & Other Agency Funding is almost
    // entirely Grants and Aid) -- without them here the chart silently
    // undercounted any department carrying those rows. Empty series are
    // dropped automatically below, so this adds nothing for departments
    // that don't have them.
    {label:'Grants and Aid',type:'Grants and Aid',color:'#9a4b3f'},
    {label:'Other Uses',type:'Other Uses',color:'#6b6f76'},
    {label:'Debt Service',type:'Debt Service',color:'#5a3d8a'}
  ];
  ...
  function bindSnapshotBudgetGraph(button,expenses,staffing,deptKey,deptLabel){
    button.addEventListener('click',function(){
      ...
      ensureChartJs().then(function(){
        renderSnapshotChart(document.getElementById(canvasId),document.getElementById(canvasId+'-legend'),expenses,showCombinedTotal);
      }).catch(function(){
  ...
  // A department page whose expense rows span more than one distinct
  // Dept_Name is really several offices/divisions rolled into one page
  // (e.g. Tourism Administration's Administration/Marketing/Sales/
  // Communications/North Walton divisions, or Code Compliance's Street/
  // Beach split) -- the flat line-item ledger below this doesn't make that
  // visible. Returns '' for a single-office department so this table only
  // shows up where it's actually informative.
  function officeSubtotalsHtml(expenses){
    var byOffice=new Map();
    (expenses||[]).forEach(function(row){
      var category=normalize(row.Object_Type||'');
      if(category!=='personnel services'&&category!=='operating expenditures') return;
      var office=String(row.Dept_Name||'').trim()||'Other';
      byOffice.set(office,(byOffice.get(office)||0)+(Number(row.FY2027_Proposed)||0));
    });
    if(byOffice.size<2) return '';
    var offices=Array.from(byOffice.keys()).sort(function(a,b){return byOffice.get(b)-byOffice.get(a);});
    var total=offices.reduce(function(runningTotal,office){return runningTotal+byOffice.get(office);},0);
    return '<p class="wc-staffing-notes-title">Subtotal by Office</p><div class="wc-data-table-scroll wc-profile-office-subtotals"><table class="wc-data-table"><thead><tr><th>Office</th><th class="wc-num">Personnel &amp; Operating</th></tr></thead><tbody>'+
      offices.map(function(office){return '<tr><td>'+escapeHtml(office)+'</td><td class="wc-num">'+money(byOffice.get(office))+'</td></tr>';}).join('')+
      '<tr class="wc-table-total-row"><td>Total</td><td class="wc-num">'+money(total)+'</td></tr></tbody></table></div>';
  }
  function bindSnapshotOperatingBudgetSheet(button,mount,departmentLabel,expenses){
    button.addEventListener('click',function(){
      if(!window.WCBudgetData||typeof window.WCBudgetData.openBudgetDetailPanel!=='function') return;
      var detail=mount&&mount.querySelector('.wc-budget-lines-detail');
  ...
  function renderSnapshotChart(canvas,legendEl,expenses,showCombinedTotal){
  ...
  // Shared by both render()'s full department-snapshot cards and
  // renderIndependentOfficeSnapshot()'s constitutional-officer/autonomous-
  // entity cards, so the row markup (tooltip anchor, share track, delta
  // pill, one-time/recurring marker) stays identical across both.
  ...
  // Capital Outlay remains available through the Capital Investments view,
  // but is intentionally omitted from the Expenditures Summary card.
  ...
  function departmentFeePayerCopy(key){
    ...
    if(key==='planning') return {label:'Short-term rental certificate and planning fees',payer:'Paid by short-term rental property owners, permit applicants, and others receiving Planning Department services.'};
  ...
  // have no SERVICES[] entry -- there's no "core services" list or
  // challenge copy written for them -- so they can't run through render()'s
  // full department-profile flow below, which bails at "if(!services)
  // return;". This builds just the Department Snapshot cards (the same
  // Expenditures/Revenue/Position card markup, buttons, and tooltips
  // render() builds for a regular department) without the Statement of
  // Function / services list / challenge-grid content that depends on
  // SERVICES data.
  function renderIndependentOfficeSnapshot(title,attempt){
    ...
    function rowsFromCard(mount){
      return Array.prototype.map.call(mount?mount.querySelectorAll('.wc-finance-card-row'):[],function(row){
        ...
        return {label:name?name.childNodes[0].textContent.trim():'Budget category',amount:amountFromText(amount&&amount.textContent),renderedChange:change&&change.textContent.trim()?{text:change.textContent.trim()}:null};
    ...
    if(keepCapitalInSummary||isAutonomousEntity) capitalGroup=null;
    ...
    revenues.forEach(function(row){var label=row.Revenue_Type||row.Revenue_Name||'Other Revenue';var item=snapshotRevenueGroups.find(function(entry){return entry.label===label;});if(!item){item={label:label,amount:0};snapshotRevenueGroups.push(item);}item.amount+=Math.abs(Number(row.FY2027_Proposed)||0);});
    snapshotRevenueGroups=snapshotRevenueGroups.filter(function(item){return item.amount>0;}).sort(function(a,b){return b.amount-a.amount;}).slice(0,4);
    if(!snapshotRevenueGroups.length) snapshotRevenueGroups=rowsFromCard(revenueMount).slice(0,4);
    var snapshotRevenueTotal=snapshotRevenueGroups.reduce(function(total,item){return total+item.amount;},0);
    ...
    var graphButton=snapshot.querySelector('[data-independent-graph-trigger]');
    if(graphButton) bindSnapshotBudgetGraph(graphButton,expenses,staffing,key,title.textContent.trim());
    var operatingButton=snapshot.querySelector('[data-independent-operating-budget-sheet-trigger]');
    if(operatingButton) bindSnapshotOperatingBudgetSheet(operatingButton,expenseMount,title.textContent.trim(),expenses);
    var capitalButton=snapshot.querySelector('[data-independent-capital-trigger]');
    if(capitalButton){
    ...
    if(contractsButton){
      ...
      bindSnapshotInformationSheet(contractsButton,'Contractual Services',contractSheetHtml,title.textContent.trim(),'wc-contract-sheet-body');
    }
    var whoPaysButton=snapshot.querySelector('[data-independent-who-pays]');
  ...
  // instead of render()'s usual one whole-page snapshot (kept in the DOM
  // but hidden for these pages -- see its own insertion point in render()).
  // Inserted right after each office's own <h2 class="tourism-admin-
  // section-title"> heading (already rendered by budget-data.js's
  // renderTourismAdministrationSections/renderTourismBeachOperationsSections
  // before render() ever reaches this point), mirroring where the
  // whole-page snapshot sits relative to the Statement of Function heading.
  function renderCombinedOfficeSnapshots(offices,deptLabel){
    offices.forEach(function(office){
      ...
      var cardExpenseGroups=capitalGroup?expenseGroups.filter(function(item){return item!==capitalGroup;}):expenseGroups;
      ...
      revenueGroups=revenueGroups.filter(function(item){return item.amount>0;}).sort(function(a,b){return b.amount-a.amount;}).slice(0,3);
      var revenueTotal=revenues.reduce(function(total,row){return total+Math.abs(Number(row.FY2027_Proposed)||0);},0);
      var whoPaysSheetHtml=fundingPayerSummary(revenues,officeKey);
      ...
      var officeGoalChain=(officeKey==='tourism administration'||officeKey==='beach operations')?officeSection.querySelector('.wc-dept-goal-chain'):null;
      if(officeGoalChain) officeGoalChain.insertAdjacentElement('afterend',snapshot); else if(narrativeBlock) narrativeBlock.insertAdjacentElement('afterend',snapshot); else if(heading) heading.insertAdjacentElement('afterend',snapshot); else officeSection.insertAdjacentElement('afterbegin',snapshot);

      bindSnapshotTooltips(snapshot);
      var graphButton=snapshot.querySelector('[data-office-graph-trigger]');
      if(graphButton) bindSnapshotBudgetGraph(graphButton,expenses,staffing,officeKey,office.label);
      ...
      // renderTourismBeachOperationsSections in budget-data.js), scoped by
      // their wc-print-kind-expense/-revenue marker classes, instead of the
      // page-level #department-expense-table/-revenue-table/-staffing-table
      // mounts the whole-page snapshot's own buttons use, which are empty
      // on these combined pages (initDepartmentPage clears them in favor of
      // the per-office cards).
      var expenseCard=officeSection.querySelector('.wc-print-kind-expense');
      var operatingButton=snapshot.querySelector('[data-office-operating-budget-sheet-trigger]');
      if(operatingButton) bindSnapshotOperatingBudgetSheet(operatingButton,expenseCard,office.label,expenses);
      var revenueCard=officeSection.querySelector('.wc-print-kind-revenue');
  ...
  // programs on one page: the main Court Technology - Court Administration
  // data (handled above like any other independent office) plus a small
  // "Court Innovations" court-cost program rendered by budget-data.js's
  // renderCourtInnovationsSupplementalTables() into its own old-style
  // finance cards (.court-innovations-cards). That block predates the
  // compact Department Snapshot card and never got migrated, so it still
  // shows the old, taller table layout. This gives it the same compact
  // card treatment (no staffing card -- Court Innovations has none) and
  // hides the old cards, matching every other office on the site.
  function renderCourtInnovationsSnapshot(){
    var section=document.querySelector('.court-innovations-supplemental-tables');
    var oldCards=document.querySelector('.court-innovations-cards');
    if(!section||!window.WCBudgetData||typeof window.WCBudgetData.getCourtInnovationsExpenses!=='function') return;
    var expenses=window.WCBudgetData.getCourtInnovationsExpenses()||[];
    var revenues=window.WCBudgetData.getCourtInnovationsRevenues()||[];
    if(!expenses.length&&!revenues.length) return;
    var expenseGroups=[
      {label:'Personnel Services',amount:sum(expenses.filter(function(row){return row.Object_Type==='Personnel Services';}),'FY2027_Proposed')},
      {label:'Operating Expenditures',amount:sum(expenses.filter(function(row){return row.Object_Type==='Operating Expenditures';}),'FY2027_Proposed')},
      {label:'Capital Outlay',amount:sum(expenses.filter(function(row){return row.Object_Type==='Capital Outlay';}),'FY2027_Proposed')}
    ].filter(function(item){return item.amount!==0;});
    if(!expenseGroups.length) expenseGroups=[{label:'Operating Expenditures',amount:sum(expenses,'FY2027_Proposed')}];
    var budget=expenseGroups.reduce(function(total,item){return total+item.amount;},0);
    var revenueGroups=[];
    revenues.forEach(function(row){var label=row.Revenue_Type||row.Revenue_Name||'Other Revenue';var item=revenueGroups.find(function(entry){return entry.label===label;});if(!item){item={label:label,amount:0};revenueGroups.push(item);}item.amount+=Math.abs(Number(row.FY2027_Proposed)||0);});
    revenueGroups=revenueGroups.filter(function(item){return item.amount>0;}).sort(function(a,b){return b.amount-a.amount;});
    var revenueTotal=revenueGroups.reduce(function(total,item){return total+item.amount;},0);
    var snapshot=document.createElement('section');
    snapshot.className='wc-profile-snapshot wc-board-department-profile wc-independent-office-snapshot';
    snapshot.innerHTML='<div class="wc-profile-snapshot-label"><h2 class="wc-profile-section-title">Court Innovations Snapshot</h2></div><div class="wc-profile-snapshot-grid wc-profile-snapshot-grid--no-staffing">'+
      '<article class="wc-profile-snapshot-card"><span class="wc-profile-snapshot-kicker">Expenditures Summary</span><div class="wc-profile-snapshot-total"><strong>'+compactMoney(budget)+'</strong></div><div class="wc-profile-snapshot-table">'+expenseGroups.map(function(item){return snapshotDeltaRow(item.label,item.amount,budget,null,false,null,null);}).join('')+'</div></article>'+
      '<article class="wc-profile-snapshot-card"><span class="wc-profile-snapshot-kicker">Revenue Summary</span><div class="wc-profile-snapshot-total"><strong>'+compactMoney(revenueTotal)+'</strong></div><div class="wc-profile-snapshot-table">'+(revenueGroups.length?revenueGroups.map(function(item){return snapshotDeltaRow(item.label,item.amount,revenueTotal,null,false,null,null);}).join(''):'<p>No dedicated revenue is listed.</p>')+'</div></article>'+
      '</div>';
    section.insertAdjacentElement('afterend',snapshot);
    if(oldCards) oldCards.classList.add('wc-independent-source-grid');
    bindSnapshotTooltips(snapshot);
  }
  ...
  function render(){
    ...
    var key=canonicalDepartmentKey(normalize(title.textContent));
    var isLifeguardProgram=key==='tourism lifeguard services and beach safety';
    ...
    // several separately budgeted offices onto one page (see
    // window.WCBudgetData.getCombinedDepartmentOffices) -- everything below
    // still builds its usual single Department Snapshot from this page's
    // combined totals (kept, just hidden -- see its own insertion point --
    // so the "Citizen questions"/cost-context content further down, which
    // is unaffected by this change, still has it to anchor its DOM
    // position off of), but a separate snapshot per office is inserted
    ...
    var servicesListHtml='<section class="wc-dept-goal-chain"><p class="wc-profile-section-title wc-dept-services-label">Department Goal and Challenges</p>'+goalHtml+challengeHtml+'<div class="wc-dept-core-services-head"><h3>Core Services &amp; Performance</h3></div>'+serviceCardsHtml+noMeasuresHtml+'<p class="wc-profile-service-note"><strong>No new services are being added.</strong> The budget continues the department&rsquo;s existing responsibilities. This list may not include all services provided by the department, but identifies its core public services and the measures used to track results.</p></section>';
    if(!isLifeguardProgram) functionSection.insertAdjacentHTML('beforeend',servicesListHtml);
    // Tourism Administration is a combined page. Place its department-wide
    // goal, services, and measures directly below the lead office statement
    // instead of after all of the tourism office sections.
    if(key==='tourism administration'){
      var tourismAdminSection=document.getElementById('tourism-administration');
      var tourismAdminNarrative=tourismAdminSection&&tourismAdminSection.querySelector('.tourism-admin-section-narrative');
      var tourismGoalChain=functionSection.querySelector('.wc-dept-goal-chain');
      if(tourismAdminNarrative&&tourismGoalChain) tourismAdminNarrative.insertAdjacentElement('afterend',tourismGoalChain);
    }
    if(key==='tourism beach operations'){
      var beachOperationsSection=document.getElementById('beach-operations');
      var beachOperationsNarrative=beachOperationsSection&&beachOperationsSection.querySelector('.tourism-admin-section-narrative');
      var beachGoalChain=functionSection.querySelector('.wc-dept-goal-chain');
      if(beachOperationsNarrative&&beachGoalChain) beachOperationsNarrative.insertAdjacentElement('afterend',beachGoalChain);
    }
    ...
    // subtraction when both are available. (snapshotDeltaRow/
    ...
    var cardPriorBudget=priorBudget-(capitalGroup?(capitalGroup.prior||0):0);
    var cardBudgetChange=cardBudget-cardPriorBudget;
    var lifeguardExpenses=[];
    var publicSafetyExpenses=[];
    function programExpenseSummary(rows){
      var groups=[
      ...
      return {groups:groups,total:total,prior:prior,change:total-prior};
    }
    var publicSafetySummary=null;
    if(isLifeguardProgram){
      lifeguardExpenses=expenses.filter(function(row){return normalize(row.Dept_Name)==='south walton fire lifeguard services';});
      publicSafetyExpenses=expenses.filter(function(row){return /^(tourism )?public safety$|sheriff beach safety/.test(normalize(row.Dept_Name));});
      var lifeguardSummary=programExpenseSummary(lifeguardExpenses);
      publicSafetySummary=programExpenseSummary(publicSafetyExpenses);
      cardExpenseGroups=lifeguardSummary.groups;
      cardBudget=lifeguardSummary.total;
      cardPriorBudget=lifeguardSummary.prior;
    ...
    snapshot.innerHTML='<div class="wc-profile-snapshot-label"><h2 class="wc-profile-section-title">'+(isLifeguardProgram?'Program Snapshot':'Department Snapshot')+'</h2></div><div class="wc-profile-snapshot-grid'+(isLifeguardProgram?' wc-profile-snapshot-grid--lifeguard':showStaffingCard?'':' wc-profile-snapshot-grid--no-staffing')+'">'+
      ...
      revenueCardHtml+
      staffingCardHtml+
      '</div>'+publicSafetySectionHtml;
    var mainContent=document.querySelector('main#content');
    // Combined pages (Tourism Administration, Tourism Beach Operations) get
    // a snapshot per office instead (see renderCombinedOfficeSnapshots
    // below) -- this combined-totals one stays in the DOM (so everything
    // below that positions itself relative to it still works unchanged)
    ...
    if(graphButton) bindSnapshotBudgetGraph(graphButton,isLifeguardProgram?lifeguardExpenses:expenses,staffing,key,title.textContent.trim());
    ...
    if(snapshotBudgetSheetButton) bindSnapshotOperatingBudgetSheet(snapshotBudgetSheetButton,document.getElementById('department-expense-table'),title.textContent.trim(),isLifeguardProgram?lifeguardExpenses:expenses);
    if(isLifeguardProgram){
      var publicSafetySource=Array.prototype.find.call(document.querySelectorAll('#department-expense-table .tourism-admin-section'),function(section){return /public safety/i.test((section.querySelector('h2')||{}).textContent||'');});
      var publicSafetyOperatingButton=snapshot.querySelector('[data-public-safety-operating-trigger]');
      var publicSafetyGraphButton=snapshot.querySelector('[data-public-safety-graph-trigger]');
      if(publicSafetyOperatingButton) bindSnapshotOperatingBudgetSheet(publicSafetyOperatingButton,publicSafetySource,'Tourism Public Safety',publicSafetyExpenses);
      if(publicSafetyGraphButton) bindSnapshotBudgetGraph(publicSafetyGraphButton,publicSafetyExpenses,[],'tourism public safety','Tourism Public Safety');
    }
